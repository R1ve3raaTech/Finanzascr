  ...
  return (
    <main className="flex min-h-[100dvh] flex-col bg-ground">
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-8 px-4 py-8 sm:px-6 sm:py-10">
        <div className="h-8 w-64 animate-pulse rounded-full bg-surface" />
        <div className="h-32 animate-pulse rounded-2xl border border-line bg-surface/60" />
        <div className="flex flex-col gap-2">
          {Array.from({ length: 5 }).map((_, i) => (
            <div
              key={i}
              className="h-[72px] animate-pulse rounded-xl border border-line bg-surface/60"
            />
          ))}
        </div>
      </div>
